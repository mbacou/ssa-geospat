...
link-citations: yes
nocite: |
  @aasr_2016, @alvarez2014typology, @jayne2015agdev, @hazell2013urban, @douillet2014developing, @jordan2005eth, @omamo2006strategic, @bacou2015ethseg, @omamo2006strategic, @benin2016agricultural, @willy2015adaptation, @jayne2015africafarmland, @azzarri2016poverty
output: 
  html_notebook:
    fig_caption: yes
    fig_height: 5
...
summaryTable <- function(dt) {
  ...
  dt <- data.table(dt)
  dt.lbl[, `Hist.` := sapply(dt, function(x) ifelse(class(x)=="numeric", toJSON(list(values=hist(x)$counts)), toJSON(NA)))]
  return(dt.lbl)
}
...
This dataset contains `r dim(hh1)[2]` variables and `r nrow(hh1)` observations. The breakdown across countries and survey years is as follows, and variables are further described in the next table.
...
# Describe variables
rhandsontable(hh1.lbl, rowHeaders=NULL) %>%
...
This dataset contains `r dim(hh2)[2]` variables. The breakdown across countries and survey years is as follows, and variables are further described in the next table.
...
# Describe variables
rhandsontable(hh2.lbl, rowHeaders=NULL) %>%
  hot_col("Hist.", renderer = htmlwidgets::JS("renderSparkline"))
...
This dataset contains `r dim(hh3)[2]` variables and `r nrow(hh3)` observations. The breakdown across countries and survey years is as follows, and variables are further described in the next table.
...
# Describe variables
rhandsontable(hh3.lbl[1:150], rowHeaders=NULL) %>%
  hot_col("Hist.", renderer = htmlwidgets::JS("renderSparkline"))
